# Test Plan: View Layer – Object Visibility When Linked to Multiple Collections (One Disabled, One Enabled)

## Description
This test validates that an object linked to two scene collections, both linked to the view layer, remains visible as long as at least one of those collections is enabled. It creates two scene collections (`Mom` and `Kid`), links the same cube to both, links both collections to a fresh view layer, disables `Mom`, enables `Kid`, updates the dependency graph, and checks that the cube is visible.
## Objective
Ensure that an object's visibility in a view layer is determined by the logical OR of its parent collections' enabled states: if any collection containing the object is enabled, the object becomes visible. The test verifies that disabling one collection while keeping another enabled does not hide the object.
## Scope
### In Scope
...
- Disabling `Mom` (`enabled = False`) and enabling `Kid` (`enabled = True`).
    
- Updating the dependency graph.
    
- Asserting that the cube is visible (`cube.visible_get()`).
### Out of Scope
- Both collections disabled (should hide object).
...
- **Execution Method:** Automated Python test
	...
	- **Test Path:** `blender/tests/python/view_layer/test_evaluation_visibility_a.py`
## Approach
...
## Test Artifact
- **Automated Script:** `blender/tests/python/view_layer/test_evaluation_visibility_a.py`
